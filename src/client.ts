import { ManagementV1DevsyWorkspaceInstanceTasks } from "@gen/models/managementV1DevsyWorkspaceInstanceTasks"
...
class Client {
  ...
  private setAccessKey(key: string) {
    ...
    Cookies.set(DEVSY_ACCESS_KEY_IDENTIFIER, key, CookieOptions)
  }
  ...
  public async socket(path: string, protocols: string[] | string | undefined): Promise<WebSocket> {
    ...
    return new Promise<WebSocket>((resolve, reject) => {
      ...
      client.onerror = (err) => {
        if (!resolved) {
          const isAuthenticated = Cookies.get(DEVSY_ACCESS_KEY_IDENTIFIER) != ""
          if (isAuthenticated) {
            reject(err)
          } else {
      ...
      if (response.status === 404) {
        ...
        if (path.includes("kiosk.sh") || path.includes("cluster.devsy.sh")) {
          ...
          if (
            ...
            return Return.Failed(
              `Agent seems to be currently unavailable, it is maybe just starting up. Click <a href="/spaces/${splitted[3]}/devsy">here</a> for more information</span>`,
  ...
  private clearStorage() {
    ...
    Cookies.remove(DEVSY_ACCESS_KEY_IDENTIFIER, { secure: true })
  ...
  private refreshCookie() {
    const accessKey = this.accessKey || localStorage.getItem(DEVSY_ACCESS_KEY_IDENTIFIER)
    const cookieAccessKey = Cookies.get(DEVSY_ACCESS_KEY_IDENTIFIER)
    if (accessKey && accessKey != cookieAccessKey) {
      Cookies.set(DEVSY_ACCESS_KEY_IDENTIFIER, accessKey, CookieOptions)
    }
  }
...
class Request<T> {
  ...
  public async DevsyWorkspaceInstanceLogs(
    namespace: string,
    instance: string,
    task: string,
    options?: LogOptions
    ...
    let requestPath = [
      this.options.basePath,
      `apis/management.devsy.sh/v1/namespaces/${namespace}/devsyworkspaceinstances/${instance}/log`,
    ].join("/")
  ...
  public async DevsyWorkspaceInstanceTasks(
    namespace: string,
    instance: string
  ): Promise<Result<ManagementV1DevsyWorkspaceInstanceTasks>> {
    let requestPath = [
      this.options.basePath,
      `apis/management.devsy.sh/v1/namespaces/${namespace}/devsyworkspaceinstances/${instance}/tasks`,
    ].join("/")
